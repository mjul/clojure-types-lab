...
(tc/cf (with-meta (constantly true) {:description "constant"}))


  ;; ; Type Error (/home/mjul/src/hypercontracts/.calva/output-window/output.calva-repl:1669:8) 
  ;; ; Polymorphic function with-meta could not be applied to arguments:
  ;; ; Polymorphic Variables:
	;; x :< IObj
  ;; 
  ;; Domains:
	;;   x (tc/Nilable (tc/Map tc/Any tc/Any))
  ;; 
  ;; Arguments:
  ;; 	[tc/Any * -> true] (tc/HMap :mandatory {:description (tc/Val "constant")} :complete? true)
  ;; 
  ;; Ranges:
  ;; 	x
  ;; 
